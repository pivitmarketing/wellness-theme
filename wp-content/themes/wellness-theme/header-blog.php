    ...
    <div class="container main-nav">
      ...
      <?php if (!is_front_page()): $extraMargin = (get_field('background_image', $pageId)) ? 'more-margin' : ''; ?>
        <h1 class="subpage-title <?php echo $extraMargin; ?>">
          <?php $title = (!is_archive() && !is_category()) ? single_post_title() : (is_search()) ? sprintf( __( '%s Search Results for ', 'html5blank' ), $wp_query->found_posts) : (is_category()) ? 'Categories' : (is_archive()) ? 'Categories/Archives' : ''; echo $title; ?>
        </h1>
      <?php endif; ?>
